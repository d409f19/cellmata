...
class Driver(private val dims: List<Int>, private val program: IProgram, private val worldType: IWorldType) {
    var worldCurrent = World(dims)
    var worldNext = World(dims)
                    ...
                    worldNext.setCell(
                        *currentPos.toIntArray(),
                        state = program.updateCell(
                            worldView = worldType.getWorldView(worldCurrent, dims, currentPos)
                        )
                    )
                    //iterate(dims.subList(1, dims.size), world[x] as MutableList<Any>)
                    x--
                }
            } else {
                var x = subDims[0] - 1
